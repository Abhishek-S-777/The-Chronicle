...
class PayRazor extends StatefulWidget {
  ...
  PayRazor({Key key, this.model1,this.addressId, this.totalAmount,this.isServiceOpted, this.subType}) : super(key: key);
...
class _PayRazorState extends State<PayRazor> {
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) async {
    ...
    if(widget.isServiceOpted==true){
      ...
      FirebaseFirestore.instance.collection("users").doc(respawn.sharedPreferences.getString(respawn.userUID)).update({
        "subscriptionStatus" : widget.subType,
      });
  ...
  void initState() {
    ...
    options = {
      ...
      'description': 'The Chronicle Payment',
      'prefill': {
        'contact': '8217748586',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("The Chronicle Payment"),
      ),
      body: Padding(
